<?php
/**
 * Sample: cancelling a Payment Order
 *
...
$cancel_payment_order_request = $mymoid_client->cancelPaymentOrderRequest($payment_order_id);
$response_object = $cancel_payment_order_request->handle()->getResponse();

// Check if response_object is an instance of PaymentOrder
if (!$response_object instanceof PaymentOrder) {
    echo 'Error cancelling: ' . $response_object->getMessage() . PHP_EOL;
    ...
    if (is_array($response_object->getDetails())) {
        foreach ($response_object->getDetails() as $error) {
            echo 'Error cancelling: ' . $error->getMessage() . PHP_EOL;
        }
    }
